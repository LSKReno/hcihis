...
export default {
  ...
  methods: {
    ...
    makePatientVisited (patient) {
      this.tableDataWaitedPatient = this.tableDataWaitedPatient.filter(item => {
        return item.registeredID !== patient.registeredID
      })
      this.tableDataVisitedPatient.unshift(patient)
    },
    getPatientInfo (row) {
