<template>
  <div>
    <ToolBar>
      <cancelRegistrationAdder
        :title="cancelRegistrationAdderTitle"
        :realName="realName"
        :identityNumber="identityNumber"
        :homeAddress="homeAddress"
        @cancel="dialogFormVisible = false"
        @refreshCancelRegistrations="refreshRegistration"
        @searchRegistrationByMedicalRecordID="searchRegistrationByMedicalRecordID"
      ></cancelRegistrationAdder>
    ...
    <div class="animated fadeInUp">
      ...
      >
        ...
        <el-table-column prop="registeredID" label="挂号ID" width="155" sortable></el-table-column>
...
// import { cancelRegistrationsTotal as cancelRegistrationsTotalApi } from "../../api/cancelRegistration";
// import { allCancelRegistration as cancelRegistrationApi } from "../../api/cancelRegistration";
// import { deleteCancelRegistration as deleteCancelRegistrationApi } from "../../api/cancelRegistration";
import cancelRegistrationAdder from './cancelRegistrationManipulate/cancelRegistrationAdder.vue'
...
export default {
  data () {
    return {
      cancelRegistrationAdderTitle: '退号',
      dialogFormVisible: true,
      patientRefundTitle: '患者退药费',
      realName: '',
      identityNumber: '',
      homeAddress: '',

      patientRefundEditChange: '',
      currentEditCancelRegistration: [],
      searchParams: {
        registeredID: '',
        medicalRecordID: ''
      },
  ...
  methods: {
    ...
    searchRegistrationByMedicalRecordID (data) {
      console.log(data)
      this.searchParams.medicalRecordID = data
      const tableRow = this.$refs.table.$el.querySelectorAll('tbody tr')
      const tableRowHeight = tableRow[1].offsetHeight
      let isjump = false
      for (let i = 0; i < this.tableDataRegistrations.length; i++) {
        if (
          ...
          this.tableDataRegistrations[i].medicalRecordID.indexOf(
            this.searchParams.medicalRecordID
          ) !== -1
        ) {
          this.realName = this.tableDataRegistrations[i].realName
          this.identityNumber = this.tableDataRegistrations[i].identityNumber
          this.homeAddress = this.tableDataRegistrations[i].homeAddress
    ...
    handleCurrentChange (val) {
      this.pageNum = val
      this.refreshCancelRegistration()
    },
    ...
    patientRefund () {
      const self = this
      self.dialogFormVisiblePatientRefund = true
    },
    handleClick (row) {
      this.$alert(row, '退号', {
          ...
          deleteRegistrationApi({
            ...
            .then(res => {
              if (res[0].status === 200) {
                self.$notify({
                  title: '成功',
                  message: '挂号名称为 ' + row.realName + ' 的挂号成功退号',
                  type: 'success',
                  offset: 60
                })
