      ...
      <el-tabs v-model="activeTabName" @tab-click="changeTab">
        <el-tab-pane label="本人" name="doctor">
          <el-collapse v-model="activePatientID" @change="handlePatientChange" accordion>
            <el-collapse-item v-for="(patient, i) in filterPatientData" :name="i" :key="i">
              <template slot="title">
                {{ patient.patientName }} {{ patient.patientStatus }}
                <div v-if="patient.patientStatus==='诊断中'">
...
<script>
import {
  getPatientByDoctor as getPatientByDoctorApi,
  getPatientByDepartment as getPatientByDepartmentApi
} from '../../../api/doctor'
